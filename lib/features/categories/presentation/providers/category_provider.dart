...
class CategoryNotifier extends Notifier<List<CategoryModel>> {
  ...
  static final List<CategoryModel> _defaultCategories = [
    ...
    CategoryModel(id: '8', name: 'Market', icon: Icons.local_grocery_store, color: Colors.green, isIncome: false),
  ...
  Future<void> _loadCategories() async {
    ...
    final dbCategories = await db.getAllCategories(user.id);
  ...
  Future<void> updateCategory(String id, {String? name, IconData? icon, Color? color}) async {
    ...
    final db = ref.read(databaseProvider);
    final dbList = await db.getAllCategories(user.id);
