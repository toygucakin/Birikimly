...
- **İşlem Sihirbazı Taşma (Pixel Overflow) Düzeltmesi:** Gelir/gider ekleme sihirbazında (`TransactionWizard`) tarih ve açıklama adımlarında yaşanan `BOTTOM OVERFLOWED` dikey taşma hatası, adım dikey alanı `130`'dan `150`'ye çıkarılarak ve adımlar `SingleChildScrollView` ile sarmalanarak çözüldü. Ayrıca klavye açıldığında diyalog yüksekliğinin dinamik olarak sınırlanması (`maxHeight` ve `maxAvailableStepHeight` insets hesaplaması) sağlanarak klavyenin sihirbazı dikeyde sıkıştırıp `BOTTOM OVERFLOWED BY 66 PIXELS` hatası vermesi kesin olarak engellendi.
